...
export default function CashierPosTerminalPage() {
  ...
  const [staffList] = useState<User[]>(() => db.getUsers().filter((u) => u.role === "staff"));
  const [selectedStaffId, setSelectedStaffId] = useState<string>(staffList[0]?.id || "usr-staff-01");

  // Master States
  const [categories, setCategories] = useState<Category[]>(() => db.getCategories(business?.id));
  const [menuItems, setMenuItems] = useState<MenuItem[]>(() => db.getMenuItems(business?.id));
  const [activeCategory, setActiveCategory] = useState<string>("all");
  const [activeShift, setActiveShift] = useState<Shift | null>(() => db.getActiveShift(business?.id));
  ...
  const [taxRatePct, setTaxRatePct] = useState<number>(0); // 0% or 10% PB1
  ...
  // Incoming QR Orders Queue
  const [pendingQrOrders, setPendingQrOrders] = useState(() => db.getPendingQrOrders(business?.id));

  const refreshAll = () => {
    setCategories(db.getCategories(business?.id));
    setMenuItems(db.getMenuItems(business?.id));
    setActiveShift(db.getActiveShift(business?.id));
    setPendingQrOrders(db.getPendingQrOrders(business?.id));
  };
  ...
  // Loyalty Customer Search in Payment Modal
  const loyaltySearchResults = useMemo(() => {
    return db.searchCustomers(business?.id, loyaltySearchQuery);
  }, [loyaltySearchQuery, business?.id]);
  ...
  const handleAcceptQrOrder = (qrOrder: Order & { items: OrderItem[] }) => {
    db.updateOrderStatus(qrOrder.id, "paid");
    refreshAll();
  ...
  const handleOpenShift = (e: React.FormEvent) => {
    e.preventDefault();
    const newShift = db.openShift(business?.id, selectedStaffId, shiftOpeningCashInput, "Shift Kasir");
  ...
  const handleCloseShift = (e: React.FormEvent) => {
    ...
    const closed = db.closeShift(activeShift.id, shiftClosingCashInput, "Tutup Shift");
    setActiveShift(null);
    setShowShiftModal(false);
    ...
    if (closed) {
      alert(
        ...
        `Modal Awal: ${formatRupiah(closed.opening_cash)}\n` +
        `Uang Sistem: ${formatRupiah(closed.expected_cash || 0)}\n` +
        `Uang Fisik: ${formatRupiah(closed.closing_cash || 0)}\n` +
        `Selisih Laci: ${formatRupiah(closed.variance || 0)} (${closed.variance === 0 ? "PAS ✓" : closed.variance! > 0 ? "LEBIH" : "KURANG"})`
      );
    }
  };
  ...
  const handlePrintBluetoothThermal = async () => {
    ...
    const staff = staffList.find((s) => s.id === completedOrder.order.created_by);
    const receiptText = generateEscPosReceiptText({
      businessName: business?.name || "KAEL POS",
      businessAddress: business?.address || "",
      businessPhone: business?.phone || "",
      orderNo: completedOrder.order.order_no,
      tableNo: completedOrder.order.table_no,
      channel: completedOrder.order.channel,
      cashierName: staff?.name || "Kasir",
      createdAt: completedOrder.order.created_at,
      ...
      subtotal: completedOrder.order.subtotal,
      discount: completedOrder.order.discount,
      tax: completedOrder.order.tax,
      serviceCharge: completedOrder.order.service_charge,
      total: completedOrder.order.total,
      paymentMethod: completedOrder.order.payment_method,
      cashGiven: completedOrder.order.cash_given || undefined,
      cashChange: completedOrder.order.cash_change || undefined,
    ...
    try {
      if ((navigator as any).bluetooth) {
        ...
        window.open(`/receipt/${completedOrder.order.id}`, "_blank");
      }
    } catch (err: any) {
      // Fallback
      window.open(`/receipt/${completedOrder.order.id}`, "_blank");
  ...
  return (
    <div className="min-h-screen bg-[#f7f6fc] text-[#232331] font-sans flex flex-col h-screen overflow-hidden">
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <span className="text-[10px] sm:text-[11px] text-[#7b7b8e] font-mono block truncate">
                Shift: {activeShift ? `Aktif (${formatRupiah(activeShift.opening_cash)})` : "Belum Dibuka"}
              </span>
            </div>
          </div>
      ...
      <main className="flex-1 grid grid-cols-12 overflow-hidden">
        ...
        {/* Left Side (7 or 8 cols): Visual Category & Menu Grid */}
        <section className="col-span-12 lg:col-span-7 xl:col-span-8 flex flex-col border-r-2 border-[#232331] bg-[#f7f6fc] overflow-hidden">
          ...
          {/* Menu Grid (Tactile Touch Buttons >= 44px) */}
        ...
        {/* Right Side (5 or 4 cols): Sticky Live Cart & Checkout */}
          ...
          <div className="w-full max-w-lg rounded-3xl border-2 border-[#232331] bg-white p-5 sm:p-6 shadow-ink-lg space-y-4 animate-in zoom-in-95 font-mono text-xs max-h-[95vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between border-b border-[#dedee8] pb-3">
              <h3 className="font-extrabold text-base text-[#232331] font-sans">
                Pembayaran Kasir #{generateDailyOrderNo(db.getOrders(business?.id).length + 1)}
              </h3>
              <button
                type="button"
            ...
            <form onSubmit={handleProcessPayment} className="space-y-4 font-sans text-xs">
              ...
              {/* Loyalty Customer Attacher (Search 4-digit WA) */}
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  className="btn-tactile w-full flex items-center justify-center gap-2 rounded-2xl border-2 border-[#232331] bg-[#16a34a] py-3.5 text-sm font-black text-white shadow-ink-md"
                >
                  <Check size={16} />
                  <span>Proses Pembayaran Selesai ✓</span>
                </button>
              </div>
          ...
          <div className="w-full max-w-sm rounded-3xl border-2 border-[#232331] bg-white p-6 shadow-ink-lg text-center space-y-4 animate-in zoom-in-95 font-mono text-xs">
            ...
            <div className="space-y-1">
              ...
              <h3 className="text-2xl font-black text-[#232331]">
                #{completedOrder.order.order_no}
              </h3>
              <p className="text-base font-extrabold text-[#16a34a]">
                Total: {formatRupiah(completedOrder.order.total)}
              </p>
              {completedOrder.order.payment_method === "cash" && (
                <p className="text-xs text-[#7b7b8e]">
                  Kembalian: {formatRupiah(completedOrder.order.cash_change || 0)}
                </p>
              )}
            </div>
            ...
            <div className="space-y-2 pt-2">
              ...
              <Link
                href={`/receipt/${completedOrder.order.id}`}
                target="_blank"
      ...
      {/* ============================================================= */}
      {/* MODAL: SHIFT MANAGEMENT (OPEN / CLOSE SHIFT) */}
      {/* ============================================================= */}
      {showShiftModal && (
        <div className="fixed inset-0 z-50 bg-[#232331]/60 backdrop-blur-xs flex items-center justify-center p-4">
            ...
            {activeShift ? (
              <form onSubmit={handleCloseShift} className="space-y-3 font-sans">
                <div className="rounded-xl bg-[#f0edff] p-3 space-y-1 font-mono text-xs">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold">{formatRupiah(activeShift.opening_cash)}</span>
                  </div>
                </div>
              ...
              <form onSubmit={handleOpenShift} className="space-y-3 font-sans">
                ...
                <div className="flex justify-end gap-2 pt-2 font-mono">
                  ...
                  <button
                    type="submit"
                    className="btn-tactile rounded-xl bg-[#232331] px-5 py-2 font-black text-[#d9ff57] shadow-ink-xs"
                  >
                    Buka Shift Sekarang ✓
                  </button>
                </div>
