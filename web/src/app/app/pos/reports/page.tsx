...
import { db, Order, Shift, Business } from "@/lib/db";
...
export default function PosOwnerReportsPage() {
  ...
  const [reports, setReports] = useState(() => db.getPosReports(business?.id));
  const [orders, setOrders] = useState<Order[]>(() => db.getOrders(business?.id));
  const [shifts, setShifts] = useState<Shift[]>(() => db.getShifts(business?.id));
  ...
  const refreshAll = () => {
    setReports(db.getPosReports(business?.id));
    setOrders(db.getOrders(business?.id));
    setShifts(db.getShifts(business?.id));
  };
  ...
  const handleProcessRefund = (e: React.FormEvent) => {
    ...
    const res = db.refundOrder(refundingOrderId, refundAmount, refundReason, "usr-owner-01");
    if (res.success) {
      refreshAll();
      setRefundingOrderId(null);
      alert("Refund berhasil diproses! Transaksi asli ditandai sebagai 'refunded' dan tercatat di audit owner.");
    } else {
      alert(res.error || "Gagal memproses refund.");
    }
  };
