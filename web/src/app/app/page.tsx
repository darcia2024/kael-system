...
import { db, Business, BusinessModule, User } from "@/lib/db";
import { formatBusinessDate } from "@/lib/formatters";

export default function AppPortalHub() {
  const [business, setBusiness] = useState<Business | null>(db.getBusiness());
  const [modules, setModules] = useState<BusinessModule[]>(db.getModules());
  ...
  const handleAddStaff = (e: React.FormEvent) => {
    ...
    if (business) {
      db.createStaff(business.id, newStaffName, newStaffPin);
      setUsers(db.getUsers());
      setShowAddStaff(false);
      setNewStaffName("");
      setNewStaffPin("");
      alert(`Staf "${newStaffName}" berhasil ditambahkan dengan PIN ${newStaffPin}.`);
    }
  };

  const toggleStaffActive = (userId: string, currentStatus: boolean) => {
