...
import { db, Card, Business } from "@/lib/db";
...
export default function KaelReviewOwnerDashboard() {
  ...
  // Get raw taps and compute cleaned metrics
  const rawTaps = db.getCardTaps();
  ...
  const handleSaveCardEdits = (e: React.FormEvent) => {
    ...
    db.updateCard(selectedCardForEdit.id, {
      label: editLabel.trim() || selectedCardForEdit.label,
    ...
    setTimeout(() => {
      ...
      alert("Pengaturan kartu berhasil diperbarui!");
    }, 300);
  };
  ...
  const handleToggleSuspend = (card: Card) => {
    ...
    if (confirm(confirmMsg)) {
      db.updateCard(card.id, { status: nextStatus });
      setCards(db.getCards());
    }
  };

  const handleSimulateLiveTap = async (cardCode: string, source: "nfc" | "qr") => {
    const card = db.getCardByCode(cardCode);
    if (!card) return;

    await db.recordCardTap(
      card.id,
      source,
      `192.168.1.${Math.floor(1 + Math.random() * 254)}`,
      "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X)"
    );

    setCards([...db.getCards()]);
    alert(`Simulasi tap ${source.toUpperCase()} pada kartu ${cardCode} berhasil dicatat dan diverifikasi!`);
  };
                  ...
                  return (
                    <tr key={card.id} className="hover:bg-[#fcfcfe]">
                      ...
                      <td className="py-3 px-3 text-right space-x-1.5 whitespace-nowrap">
                        ...
                        <button
                          type="button"
                          onClick={() => handleSimulateLiveTap(card.card_code, "nfc")}
                          className="btn-tactile rounded-lg border border-[#232331] bg-[#d9ff57] px-2 py-1 text-[10.5px] font-bold text-[#232331] shadow-ink-xs"
                          title="Simulasikan 1x Tap NFC dari Pelanggan"
                        >
                          Simulasi Tap
                        </button>
