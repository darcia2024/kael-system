...
export default function KaelLoyaltyDashboard() {
  ...
  // Master States
  const [program, setProgram] = useState<LoyaltyProgram>(() => db.getLoyaltyProgram());
  ...
  const searchResults = useMemo(() => {
    return db.searchCustomers(business?.id, searchQuery);
  }, [searchQuery, customers, business?.id]);

  // Selected customer live balance
  const activeCustomerBalance = useMemo(() => {
    return selectedCustomer ? db.getCustomerPointBalance(selectedCustomer.id) : 0;
  ...
  const refreshAll = () => {
    ...
    setProgram(db.getLoyaltyProgram());
  };
  ...
  const handleAddPoints = (e: React.FormEvent) => {
    ...
    db.addPointTransaction(
      ...
      earned,
      "purchase",
      `Transaksi Kasir (${formatRupiah(amountSpentInput)})`,
      amountSpentInput,
      selectedStaffId
    );

    refreshAll();
    setCashierSuccessMsg(`Sukses! +${earned} ${program.mode === "stamp" ? "Stamp" : "Poin"} berhasil ditambahkan untuk ${selectedCustomer.name}.`);
  ...
  const handleRedeemReward = () => {
    ...
    if (confirm(`Konfirmasi penukaran: Tukar ${reward.point_cost} Poin untuk "${reward.name}"?`)) {
      const res = db.issueRedemption(selectedCustomer.id, selectedRewardId, selectedStaffId);
      if (res.success && res.redemption) {
        refreshAll();
        setLastRedemptionResult(res.redemption);
        setCashierSuccessMsg(`Sukses! Voucher ${reward.name} berhasil diterbitkan (Kode: ${res.redemption.code}).`);
      } else {
        alert(res.error || "Gagal memproses penukaran.");
      }
    }
  };
  ...
  const handleAnonymizeCustomer = (id: string, name: string | null) => {
  ...
  const handleSaveProgramSettings = (e: React.FormEvent) => {
    e.preventDefault();
    db.updateLoyaltyProgram(business?.id, program);
    refreshAll();
    alert("Pengaturan program loyalitas berhasil disimpan!");
  };
      ...
      <main className="flex-1 mx-auto w-full max-w-6xl p-3 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4">
          ...
          <div className="card-tactile rounded-2xl sm:rounded-3xl border sm:border-2 border-[#232331] bg-white p-3.5 sm:p-5 shadow-ink-xs sm:shadow-ink-md">
            ...
            <div className="mt-2 sm:mt-3">
              <h3 className="text-lg sm:text-3xl font-extrabold font-mono text-[#232331]">
                {customers.reduce((acc, c) => acc + db.getCustomerPointBalance(c.id), 0)} Pts
              </h3>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left font-mono text-xs">
                ...
                <tbody className="divide-y divide-[#dedee8]">
                  {customers.map((cust) => {
                    const balance = db.getCustomerPointBalance(cust.id);
        ...
        {activeTab === "audit" && (
          <div className="rounded-2xl sm:rounded-3xl border sm:border-2 border-[#232331] bg-white p-4 sm:p-6 shadow-ink-md space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-3 font-mono text-xs">
              {db.getStaffPointsAudit(business?.id).map((audit) => (
                <div key={audit.staffId} className="rounded-2xl border-2 border-[#232331] bg-[#fcfcfe] p-4 space-y-2 shadow-ink-xs">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-[10px] bg-[#dcfce7] text-[#16a34a] px-2 py-0.5 rounded font-bold">
                      {audit.transactionCount} Transaksi
                    </span>
                  </div>
                  ...
                  <div className="rounded-xl bg-white p-2.5 border border-[#dedee8] space-y-1 text-[11px]">
                    <div className="flex justify-between">
                      ...
                      <span className="font-black text-[#7958d8]">{audit.totalPointsIssued} Pts</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[#7b7b8e]">Penyesuaian Manual:</span>
                      <span className="font-bold text-[#c2410c]">{audit.manualPointsCount} Pts</span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="w-full max-w-lg rounded-3xl border-2 border-[#232331] bg-white p-6 shadow-ink-lg space-y-4 animate-in fade-in font-mono text-xs">
            <div className="flex items-center justify-between border-b border-[#dedee8] pb-3">
              <div>
                ...
                <span className="text-[11px] text-[#7b7b8e]">
                  Saldo: {db.getCustomerPointBalance(viewingCustomer.id)} Pts · {maskPhoneNumber(viewingCustomer.phone)}
                </span>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
              {db.getCustomerLedger(viewingCustomer.id).map((item) => (
