...
import { db, MenuItem, Category, Business } from "@/lib/db";
...
export default function CustomerQrOrderPage() {
  ...
  const categories: Category[] = useMemo(() => db.getCategories(business?.id), [business?.id]);
  const menuItems: MenuItem[] = useMemo(() => db.getMenuItems(business?.id), [business?.id]);
  ...
  const handleCheckout = (e: React.FormEvent) => {
    ...
    // Register or get customer if phone provided
    let customerId: string | null = null;
    if (customerPhone.trim()) {
      const custRes = db.registerCustomer(business?.id, customerName.trim() || "Tamu Meja " + tableNo, customerPhone.trim());
      if (custRes.customer) customerId = custRes.customer.id;
    }

    const res = db.createOrder(
      business?.id,
      {
        channel: "qr_dinein",
        table_no: tableNo,
        status: "open", // masuk antrean dapur kasir
        payment_method: "qris",
    ...
    setCompletedOrderNo(res.order.order_no);
    setCart({});
  };
