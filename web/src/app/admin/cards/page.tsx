...
export default function KaelAdminCardsPage() {
  const [cards, setCards] = useState<Card[]>(db.getAllCards());
  const [batchCount, setBatchCount] = useState<number>(5);
  const [batchType, setBatchType] = useState<"review" | "loyalty" | "attendance">("review");
  const [generatedBatchInfo, setGeneratedBatchInfo] = useState<{ cards: Card[]; plainPins: string[] } | null>(null);
  ...
  const handleGenerateBatch = (e: React.FormEvent) => {
    ...
    const result = db.createBatchCards(count, batchType);
    setGeneratedBatchInfo(result);
              ...
              <div className="flex items-center justify-between border-b border-[#7958d8]/30 pb-3">
                <div>
                  <span className="font-extrabold text-sm text-[#7958d8] block">
                    ✓ {generatedBatchInfo.cards.length} KARTU BARU BERHASIL DITERBITKAN
                  </span>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left bg-white rounded-xl border border-[#dedee8]">
                  ...
                  <tbody className="divide-y divide-[#dedee8]">
                    {generatedBatchInfo.cards.map((card, idx) => {
                      const pin = generatedBatchInfo.plainPins[idx];
                      return (
                        <tr key={card.id}>
                          ...
                          <td className="p-2.5 text-right">
                            <button
                              type="button"
                              onClick={() => handleCopyText(`Card: ${card.card_code} | PIN: ${pin}`, card.id)}
                              className="text-[11px] font-bold text-[#7958d8] hover:underline"
                            >
                              {copiedId === card.id ? "Disalin ✓" : "Salin Info"}
                            </button>
                          </td>
                        </tr>
                      );
