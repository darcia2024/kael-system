...
import { db, Customer, Business, LoyaltyProgram, Reward, PointLedger, Redemption } from "@/lib/db";
...
export default function CustomerMemberProgressPage() {
  ...
  const customer: Customer | null = useMemo(() => db.getCustomerByToken(token), [token]);
  const business: Business | null = useMemo(() => {
    return customer ? db.getBusiness(customer.business_id) : null;
  }, [customer]);

  const program: LoyaltyProgram = useMemo(() => {
    return customer ? db.getLoyaltyProgram(customer.business_id) : db.getLoyaltyProgram();
  }, [customer]);

  const [rewards, setRewards] = useState<Reward[]>(() => {
    return customer ? db.getRewards(customer.business_id).filter((r) => r.is_active) : [];
  });
  ...
  const balance = customer ? db.getCustomerPointBalance(customer.id) : 0;
  const ledger: PointLedger[] = customer ? db.getCustomerLedger(customer.id) : [];
  const redemptions: Redemption[] = customer ? db.getRedemptions(customer.id) : [];
